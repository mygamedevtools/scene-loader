...
`SceneManagerLog` é o destino único de todo diagnóstico que o pacote emite. Ele existe para que o pacote tenha um só lugar de onde reportar, e você tenha um só lugar para controlar.
...
|---|---|
| `Off` | Não emite nada. |
| `Error` | Uma operação falhou, ou um estado do qual o manager depende está inconsistente. |
| `Warning` | Algo recuperável, ou uma API usada de um jeito que não vai fazer o que quem chamou espera. |
| `Info` | Progresso grosseiro ao longo de uma operação. |
| `Verbose` | Detalhe passo a passo, para diagnosticar uma falha específica. |

`Level` tem como padrão **`Warning` em builds de desenvolvimento** e **`Error` em release** — ou seja, `Debug.isDebugBuild`, então ele acompanha o editor e o player de desenvolvimento automaticamente.

A filtragem é **inteiramente em runtime**. Não há chave de compilação, de propósito: poder aumentar o nível de log dentro de uma build que você já lançou é exatamente a situação para a qual isso vale a pena.
...
São intenções diferentes, e misturá-las faria um `null` acidental parecer um botão de desligar funcionando.
:::

:::info
Um handler que lança exceção é contido e reportado ao console da Unity no lugar. Um sink de analytics quebrado não vai derrubar o carregamento de cena que estava tentando reportar por ele.
:::

## O que custa o quê {/* #what-costs-what */}

Como a filtragem é em runtime, a mensagem é construída no ponto de chamada, seja ela emitida ou não. O pacote protege o punhado de pontos que rodam por operação ou por cena, e deixa o resto sem proteção — uma mensagem que dispara uma vez ou nunca não vale um `if`.
